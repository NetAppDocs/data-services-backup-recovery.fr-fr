...
[role="lead"]
Utilisez NetApp Backup and Recovery pour gérer les sauvegardes de snapshots, la réplication et les sauvegardes sur stockage d’objets pour vos systèmes Cloud Volumes ONTAP et ONTAP sur site. Vous pouvez modifier les planifications, activer ou désactiver les sauvegardes de volumes, suspendre les sauvegardes, supprimer les sauvegardes et désenregistrer le service.
...
. Vérifiez l'état des options de protection dans la colonne « Protection existante ». Les trois icônes correspondent aux « Instantanés locaux », aux « Volumes répliqués » et aux « Sauvegardes dans le stockage objet ».
+
...
Consultez link:br-start-enable.html["Activez NetApp Backup and Recovery pour protéger les volumes"^] pour obtenir des instructions.
...
.Étapes
. Dans le menu *Volumes*, repérez le volume dont vous souhaitez modifier les paramètres de stratégie, puis sélectionnez le menu Actions. image:icon-action.png["Icône Actions"] à la fin de la ligne, et sélectionnez *Modifier la stratégie de sauvegarde*.
. Sur la page _Modifier la stratégie de sauvegarde_, apportez des modifications aux politiques de sauvegarde existantes pour les instantanés locaux, les volumes répliqués et les fichiers de sauvegarde, puis sélectionnez *Suivant*.
+
Si vous avez activé _DataLock and Ransomware protection_ pour les sauvegardes cloud dans la règle de sauvegarde initiale lors de l’activation de NetApp Backup and Recovery pour ce cluster, vous ne verrez que les autres règles qui ont été configurées avec DataLock. Et si vous n’avez pas activé _DataLock and Ransomware protection_ lors de l’activation de NetApp Backup and Recovery, vous ne verrez que les autres règles de sauvegarde cloud qui n’ont pas DataLock configuré.
...
. Selon le type d'action de sauvegarde que vous souhaitez configurer, sélectionnez le bouton dans le menu Actions groupées :
+
[cols="50,30"]
|===
...
Cette fonctionnalité est disponible uniquement si la sauvegarde du volume a été créée à partir d'un système doté d'ONTAP 9.11.1 ou supérieur, et si vous avez activé _DataLock et la protection contre les ransomwares_ dans la règle de sauvegarde vers objet.
...
. Sélectionnez image:icon-actions-horizontal.gif["Icône Actions"] pour le fichier de sauvegarde de volume que vous souhaitez analyser à la recherche de ransomware et sélectionnez *Analyser à la recherche de ransomware*.
+
La colonne de protection contre les ransomwares indique que l'analyse est en cours.
...
[NOTE]
====
* Si vous avez activé _DataLock and Ransomware protection_ dans la règle initiale lors de l’activation de NetApp Backup and Recovery pour ce cluster, toutes les règles que vous modifiez doivent être configurées avec le même paramètre DataLock (Governance ou Compliance). Et si vous n’avez pas activé _DataLock and Ransomware protection_ lors de l’activation de NetApp Backup and Recovery, vous ne pouvez pas activer DataLock maintenant.
...
+
link:prev-reference-aws-archive-storage-tiers.html["En savoir plus sur l'utilisation du stockage d'archives AWS"]. link:prev-reference-azure-archive-storage-tiers.html["En savoir plus sur l'utilisation du stockage d'archives Azure"]. link:prev-reference-gcp-archive-storage-tiers.html["En savoir plus sur l'utilisation du stockage d'archives Google"]. (Nécessite ONTAP 9.12.1.)
...
[NOTE]
====
* Si vous avez activé _DataLock and Ransomware protection_ dans la règle initiale lors de l’activation de NetApp Backup and Recovery pour ce cluster, toutes les règles supplémentaires que vous créez doivent être configurées avec le même paramètre DataLock (Governance ou Compliance). Et si vous n’avez pas activé _DataLock and Ransomware protection_ lors de l’activation de NetApp Backup and Recovery, vous ne pouvez pas créer de nouvelles règles utilisant DataLock.
...
NOTE: Vous ne pouvez pas supprimer individuellement les fichiers de sauvegarde d'un volume une fois la relation de sauvegarde supprimée. Supprimez les fichiers de sauvegarde d'un volume avant de supprimer sa relation de sauvegarde.

.Étapes
. Dans le menu *Volumes*, sélectionnez image:icon-actions-horizontal.gif["Plus d'icônes"] pour le volume source et sélectionnez *Gérer les sauvegardes > Afficher la sauvegarde*.
+
La liste de tous les fichiers de sauvegarde s'affiche.

. Choisissez une sauvegarde dans la liste et sélectionnez *Actions > Supprimer toutes les sauvegardes*.
. Confirmez la suppression en saisissant le nom du volume.
. Développez la section *Paramètres avancés*.
. Sous *Forcer la suppression des sauvegardes*, sélectionnez si vous souhaitez forcer la suppression de toutes les sauvegardes.
+
Dans de rares cas, vous pourriez avoir besoin de révoquer l'accès de NetApp Backup and Recovery aux sauvegardes' ; par exemple, si Backup and Recovery perd l'accès au compartiment de sauvegarde ou si les sauvegardes sont DataLock-protégées mais que vous ne souhaitez plus les conserver. Utilisez cette option pour forcer la suppression des sauvegardes au niveau du volume et du système.

+

CAUTION: N'utilisez cette option qu'en cas de nettoyage extrême. Une fois activée, NetApp Backup and Recovery n'accédera plus à ces sauvegardes (même si les objets restent dans le stockage cloud). Vous devez les supprimer manuellement auprès de votre fournisseur de cloud.
...
Supprimez une sauvegarde individuelle lorsqu'elle n'est plus nécessaire. Vous ne pouvez pas supprimer les volumes répliqués (volumes de protection des données).


NOTE: Vous ne pouvez pas supprimer individuellement les fichiers de sauvegarde de volume une fois que la relation de sauvegarde a été supprimée. Vous pouvez cependant <<Supprimer tous les fichiers de sauvegarde d'un volume,supprimer tous les fichiers de sauvegarde du volume>>.
...
NOTE: Vous ne pouvez pas supprimer individuellement les fichiers de sauvegarde d'un volume une fois la relation de sauvegarde supprimée. Cependant, vous pouvez <<Supprimer tous les fichiers de sauvegarde d'un volume,supprimer tous les fichiers de sauvegarde du volume>>. Si vous devez supprimer une relation de sauvegarde pour un volume, assurez-vous de supprimer d'abord les sauvegardes du volume, puis de supprimer la relation de sauvegarde.
...
La désactivation de NetApp Backup and Recovery pour un système suspend les sauvegardes et les restaurations de tous ses volumes sans supprimer les sauvegardes existantes. Cela vous permet de suspendre toute activité de sauvegarde et de restauration pendant une période déterminée sans désinscrire le service de sauvegarde de ce système.
