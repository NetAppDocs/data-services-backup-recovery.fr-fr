...
[role="lead"]
Découvrez ou ajoutez manuellement des cibles de sauvegarde hors site telles que AWS S3, Azure Blob Storage, Google Cloud Storage ou StorageGRID afin de pouvoir stocker des sauvegardes en dehors de votre environnement principal.
...
Fournissez les identifiants d'accès au compte de stockage. NetApp Backup and Recovery utilise ces identifiants pour découvrir et vérifier l'accès à vos compartiments de sauvegarde cibles.

.Avant de commencer
Vous devez découvrir au moins une charge de travail avant de pouvoir ajouter une cible de sauvegarde hors site.

.Étapes
. Dans le menu NetApp Backup and Recovery, sélectionnez *Cibles de sauvegarde*.
. Sélectionnez *Découvrir la cible de sauvegarde*.
. Sélectionnez l'un des types de cibles de sauvegarde disponibles : *AWS*, *Azure*, *StorageGRID*, *ONTAP S3* ou *GCP*.
. Sélectionnez *Suivant*.
. (Pour les cibles de sauvegarde AWS et Azure uniquement) Dans la section *Choisir l'emplacement des identifiants*, choisissez l'emplacement où se trouvent les identifiants, puis choisissez comment les associer.
. Sélectionnez *Suivant*.
...
*** *Clé d'accès* : Saisissez la clé d'accès AWS.
...
*** *ID de l'application (client)* : Saisissez l'ID de l'application Azure Blob Storage.
...
*** *Nom de domaine complet du nœud de passerelle* : Saisissez un nom de domaine complet (FQDN) pour le nœud de passerelle StorageGRID.
...
*** *Nom de domaine complet du nœud de passerelle* : Saisissez un nom de domaine complet (FQDN) pour le nœud de passerelle ONTAP S3.
...
** Pour GCP :
+
*** *Nom de l'identifiant* : Saisissez le nom de l'identifiant Google Cloud.
*** *Clé d'accès* : Saisissez la clé d'accès Google Cloud.
*** *Clé secrète* : Saisissez la clé secrète Google Cloud.
...
Ajoutez manuellement un compartiment à une cible de sauvegarde hors site au lieu de laisser NetApp Backup and Recovery découvrir automatiquement les compartiments.

.Étapes
. Dans le menu NetApp Backup and Recovery, sélectionnez *Cibles de sauvegarde*.
. Sélectionnez une cible et, à droite, sélectionnez l’icône *Actions* image:icon-action.png["Icône Actions"] puis sélectionnez *Ajouter un compartiment*.
...
** Pour Azure :
+
*** *Abonnement* : Sélectionnez le nom de l’abonnement Azure.
*** *Groupe de ressources* : sélectionnez le nom du groupe de ressources Azure.
*** *Détails de l'instance*:
+
**** *Nom du compte de stockage* : Saisissez le nom du compte de stockage Blob Azure.
**** *Région Azure* : Saisissez la région Azure du compte de stockage.
**** *Type de performances* : Sélectionnez le type de performances standard ou premium pour le compte de stockage Blob Azure, en indiquant le niveau de performances requis.
**** *Chiffrement* : Sélectionnez le type de chiffrement pour le compte Azure Blob Storage. Les options sont soit des clés gérées par Microsoft, soit des clés gérées par le client. Si vous sélectionnez des clés gérées par le client, vous devez fournir le nom du coffre de clés et le nom de la clé.
...
** Pour StorageGRID:
+
*** *Cible de sauvegarde* : Sélectionnez le nom de la cible de sauvegarde StorageGRID.
...
** Pour ONTAP S3 :
+
*** *Cible de sauvegarde* : Sélectionnez le nom de la cible de sauvegarde ONTAP S3.
*** *Nom du compartiment* : Saisissez le nom du compartiment ONTAP S3.
...
** Pour GCP :
+
*** *Nom du bucket* : Saisissez le nom du bucket Google Cloud.
*** *Projet GCP* : Sélectionnez le nom du projet Google Cloud.
*** *Région du bucket* : Sélectionnez une région pour le bucket Google Cloud.
*** *Verrouillage des objets* : Sélectionnez cette option pour activer les périodes de rétention des objets dans ce bucket. Pendant qu'une période de rétention est en vigueur, les objets ne peuvent pas être modifiés ou supprimés et sont immuables jusqu'à l'expiration de la période. Si vous activez cette option pour un bucket, elle ne peut pas être désactivée.
...
.Étapes
. Dans le menu NetApp Backup and Recovery, sélectionnez *Cibles de sauvegarde*.
. Sélectionnez une cible et, à droite, sélectionnez l’icône *Actions* image:icon-action.png["Icône Actions"] et sélectionnez *Modifier les informations d’identification*.
