...
Si vous sélectionnez un fichier de sauvegarde dans le stockage d'objets et que la protection contre les ransomwares est activée pour cette sauvegarde (si vous avez activé Datalock et la protection contre les ransomwares dans les règles de sauvegarde), vous êtes invité à effectuer une vérification d'intégrité supplémentaire sur le fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d'effectuer cette analyse.
