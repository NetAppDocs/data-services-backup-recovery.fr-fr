...
Ces deux fonctionnalités protègent vos fichiers de sauvegarde, vous assurant ainsi de toujours disposer d'un fichier de sauvegarde valide pour récupérer des données en cas de tentative d'attaque par ransomware sur vos sauvegardes. Elles sont également utiles pour répondre à certaines exigences réglementaires où les sauvegardes doivent être verrouillées et conservées pendant une certaine période. Lorsque l'option *Datalock and Ransomware protection* est activée, le compartiment cloud provisionné dans le cadre de l’activation de NetApp Backup and Recovery bénéficiera du verrouillage et du versionnage des objets.
...
. Lors de la création d'une règle, développez la section *Datalock and Ransomware protection*.
