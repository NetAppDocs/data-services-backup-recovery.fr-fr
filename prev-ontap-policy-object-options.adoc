...
[role="lead"]
Utilisez les règles de sauvegarde vers objet dans NetApp Backup and Recovery pour définir les calendriers, la rétention, DataLock, la protection contre les ransomwares et les paramètres d’archivage pour vos systèmes ONTAP sur site et Cloud Volumes ONTAP.
...
Chaque règles de sauvegarde comporte une section « Étiquettes et conservation » que vous pouvez appliquer à vos fichiers de sauvegarde. Notez que la règle de snapshot appliquée au volume doit être l'une des règles reconnues par NetApp Backup and Recovery, sinon les fichiers de sauvegarde ne seront pas créés.
...
Ces deux fonctionnalités protègent vos fichiers de sauvegarde, vous assurant ainsi de toujours disposer d'un fichier de sauvegarde valide pour récupérer des données en cas de tentative d'attaque par ransomware sur vos sauvegardes. Elles sont également utiles pour répondre à certaines exigences réglementaires lorsque les sauvegardes doivent être verrouillées et conservées pendant une certaine période. Lorsque l'option *DataLock and Ransomware protection* est activée, le compartiment cloud provisionné dans le cadre de l'activation de NetApp Backup and Recovery bénéficiera du verrouillage et du versionnage des objets.
...
. Lorsque vous créez une règle, développez la section *DataLock et protection contre les ransomwares*.
. Choisissez l’une des options suivantes :
+
** *Aucune* : la protection DataLock et la protection contre les ransomwares sont désactivées.
** *Déverrouillé* : la protection DataLock et la protection contre les ransomwares sont activées. Les utilisateurs disposant des autorisations requises peuvent écraser ou supprimer les fichiers de sauvegarde protégés pendant la période de conservation.
** *Verrouillé* : la protection DataLock et la protection contre les ransomwares sont activées. Aucun utilisateur ne peut écraser ni supprimer les fichiers de sauvegarde protégés pendant la période de conservation. Ceci garantit la conformité réglementaire totale.
...
| Cloud Volumes ONTAP dans Google Cloud | Stockage Google Cloud 
...
* Pour AWS :
+
** Vos clusters doivent exécuter ONTAP 9.11.1 ou une version ultérieure
...
Lorsque vous utilisez AWS, Azure ou Google Cloud Storage, vous pouvez déplacer les fichiers de sauvegarde les plus anciens vers une classe de stockage d’archivage ou un niveau d’accès moins coûteux après un certain nombre de jours. Vous pouvez également choisir d’envoyer vos fichiers de sauvegarde vers le stockage d’archivage immédiatement, sans qu’ils soient écrits sur le stockage cloud standard. Il vous suffit de saisir *0* comme valeur pour « Archive After Days » afin d’envoyer votre fichier de sauvegarde directement vers le stockage d’archivage. Cela peut être particulièrement utile pour les utilisateurs qui ont rarement besoin d’accéder aux données des sauvegardes cloud ou pour ceux qui remplacent une solution de sauvegarde sur bande.
...
** Pour Azure, vous pouvez hiérarchiser les anciennes sauvegardes vers le stockage _Azure Archive_. link:prev-reference-azure-archive-storage-tiers.html["En savoir plus sur le stockage d'archives Azure"^].
