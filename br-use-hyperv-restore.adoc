---
sidebar: sidebar 
permalink: br-use-hyperv-restore.html 
keywords: restore, vm, virtual machine, snapshots, object storage, cloud storage, backing up, back up, backup, Amazon Web services, AWS, NetApp Backup and Recovery 
summary: Protégez vos charges de travail Hyper-V avec NetApp Backup and Recovery. 
---
= Restaurer les charges de travail Hyper-V avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Restaurez les charges de travail Hyper-V à partir d'instantanés locaux, de sauvegardes répliquées sur un stockage secondaire ou de sauvegardes dans un stockage objet à l'aide de NetApp Backup and Recovery.

.Restaurer à partir de ces emplacements
Vous pouvez restaurer les charges de travail à partir de ces emplacements :

* Restaurer à partir d'un emplacement principal (instantané local)
* Restaurer à partir d'une ressource répliquée sur un stockage secondaire
* Restaurer à partir d'une sauvegarde de stockage d'objets


.Restaurer ces points
Vous pouvez restaurer les données vers ces destinations :

* Restaurer à l'emplacement d'origine (à partir du stockage principal, secondaire et objet)
* Restaurer vers un autre emplacement (à partir du stockage principal, secondaire et objet)


.Considérations relatives à la restauration à partir du stockage d'objets
Si vous sélectionnez un fichier de sauvegarde dans le stockage d'objets et que la protection contre les ransomwares est active pour cette sauvegarde parce que vous avez activé DataLock et la protection contre les ransomwares dans la règle de sauvegarde, vous êtes invité à effectuer une vérification d'intégrité supplémentaire avant de restaurer les données. Nous vous recommandons d'effectuer cette analyse.


TIP: Des frais de sortie supplémentaires seront facturés par votre fournisseur de cloud pour accéder au contenu du fichier de sauvegarde.



[[how-restoring-workloads-works]]
== Comment fonctionne la restauration des charges de travail

Lors de la restauration d'une charge de travail, les événements suivants se produisent :

* Lorsque vous restaurez une charge de travail à partir d'un fichier de sauvegarde local, NetApp Backup and Recovery crée une _nouvelle_ ressource à l'aide des données de la sauvegarde.
* Lorsque vous effectuez une restauration à partir d’une charge de travail répliquée, vous pouvez restaurer la charge de travail sur le système d’origine ou sur un système ONTAP local.


Depuis la page Restaurer, vous pouvez restaurer une ressource même si vous ne vous souvenez pas de son nom exact, de son emplacement ou de la date de sa dernière bonne sauvegarde. Vous pouvez rechercher l’instantané à l’aide de filtres.



[[restore-hyper-v-workloads]]
== Restaurer les charges de travail Hyper-V

Restaurez les charges de travail Hyper-V à l'aide du menu Restaurer. Vous pouvez rechercher un instantané par nom ou à l'aide de filtres.

*Rôle requis de la console NetApp* Rôle de super administrateur de NetApp Backup and Recovery ou rôle d'administrateur de restauration de NetApp Backup and Recovery. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["En savoir plus sur les rôles d'accès à la NetApp Console pour tous les services"^].

.Étapes
. Dans le menu NetApp Backup and Recovery , sélectionnez *Restaurer*.
. Dans la liste déroulante à droite du champ de recherche de nom, sélectionnez *Hyper-V*.
. Saisissez le nom de la ressource que vous souhaitez restaurer, ou filtrez par nom de machine virtuelle, hôte de machine virtuelle ou pool de stockage où se trouve la ressource.
+
Une liste d'instantanés correspondant à vos critères de recherche apparaît.

. Sélectionnez le bouton *Restaurer* pour l’instantané que vous souhaitez restaurer.
+
Une liste de points de restauration possibles apparaît.

. Sélectionnez le point de restauration que vous souhaitez utiliser.
. Sélectionnez un emplacement source d’instantané.
. Sélectionnez *Suivant* pour continuer.
. Choisissez la destination et les paramètres de restauration :
+
[discrete]
===== Sélection de la destination

+
[role="tabbed-block"]
====
.Restaurer à l'emplacement d'origine
--
.. Sélectionnez le volet *Emplacement d'origine*. Lorsque vous restaurez vers l'emplacement d'origine, vous pouvez consulter les paramètres de destination en développant la section *Paramètres de destination*, mais vous ne pouvez pas les modifier.
.. Dans la section *Options post-restauration*, considérez l'option suivante :
+
*** *Démarrer la machine virtuelle* : Activez cette option pour démarrer la nouvelle machine virtuelle après sa restauration.




--
.Restaurer vers un autre emplacement
--
.. Sélectionnez le volet *Emplacement alternatif*.
.. Dans la section *Paramètres de destination*, saisissez les informations suivantes :
+
*** *Nom de domaine complet ou adresse IP Hyper-V* : Saisissez le nom de domaine complet ou l’adresse IP de l’hôte Hyper-V de destination.
*** *Réseau* : Sélectionnez le réseau de destination sur lequel vous souhaitez restaurer l’instantané.
*** *Nom de la machine virtuelle* : Saisissez le nom de la machine virtuelle que vous souhaitez restaurer.
*** *Emplacement de destination* : Saisissez le dossier de destination ou le partage CIFS qui doit contenir les données restaurées.


.. Dans la section *Stockage de destination* (visible uniquement lors de la restauration à partir du stockage d’objets) :
+
*** *Modifier l'emplacement de stockage par défaut* : activez cette option pour restaurer une sauvegarde d'objets vers un autre emplacement si l'emplacement de stockage source est inaccessible ou saturé. Choisissez un autre cluster, une autre machine virtuelle de stockage ou un autre agrégat à utiliser.


.. Dans la section *Options de pré-restauration*, prenez en compte les options suivantes :
+
*** *Restauration rapide* : Activez cette option pour rendre la machine virtuelle restaurée immédiatement disponible. Seuls les fichiers nécessaires au fonctionnement de la machine virtuelle sont restaurés à partir du stockage d'objets, et non la totalité du volume.


.. Dans la section *Options post-restauration*, prenez en compte les options suivantes :
+
*** *Démarrer la machine virtuelle* : Activez cette option pour démarrer la nouvelle machine virtuelle après sa restauration.




--
====
. Sélectionnez *Restaurer*.

