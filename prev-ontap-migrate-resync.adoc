...
[role="lead"]
Utilisez SnapMirror to Cloud Resync dans NetApp Backup and Recovery pour conserver les sauvegardes cloud existantes disponibles après la migration d’un volume avec la réplication logique SnapMirror (LRSE). Cette fonctionnalité permet la poursuite des sauvegardes sans nouvelle ligne de base et est disponible à partir de ONTAP 9.16.1.

SnapMirror to Cloud Resync prend en charge les migrations FlexVol et FlexGroup et vous aide à maintenir la continuité des sauvegardes dans les environnements hybrides et multicloud.
...
Imaginez que vous disposez d’un volume local appelé Vol1a. Ce volume comporte trois snapshots : S1, S2 et S3. Ces snapshots sont des points de restauration. Vol1a est sauvegardé dans le cloud à l’aide de SnapMirror to Cloud (SM-C), mais seuls S1 et S2 se trouvent dans le stockage d’objets.
...
NOTE: N’utilisez pas de stratégie SnapMirror de type « Vault » pour cloner un volume d’un ancien cluster ONTAP vers un nouveau cluster ONTAP ou dans une architecture en cascade. Les stratégies Vault modifient l’UUID de version du snapshot lors du transfert. L’UUID de version du snapshot est utilisé pour déterminer si un snapshot commun existe entre la source et la destination.
...
+
NetApp Backup and Recovery valide que chaque volume sélectionné pour la resynchronisation partage un instantané commun avec le stockage d'objets. Plus précisément, le dernier instantané stocké dans le stockage d'objets doit également exister dans l'instantané local du nouveau volume cloné. Cette vérification garantit que les volumes sont prêts pour l'opération SnapMirror to Cloud Resync.
...
. Vérifiez l'éligibilité des volumes. Le système vérifie si les volumes sont éligibles à la resynchronisation. Si un volume n'est pas éligible, cela signifie que les critères communs de création d'instantanés n'ont pas été respectés.
+
