...
Cette fonctionnalité élimine le besoin d'un processus de réétablissement de la configuration de référence et permet aux sauvegardes de se poursuivre après la migration. Cette fonctionnalité est utile dans les scénarios de migration de charge de travail, prenant en charge à la fois FlexVols et FlexGroups, et est disponible à partir de la version 9.16.1 ONTAP .
...
. Vérifiez l'éligibilité des volumes. Le système vérifie si les volumes sont éligibles à la resynchronisation. Si un volume n'est pas éligible, cela signifie que les critères de snapshot commun n'ont pas été établis.
+
