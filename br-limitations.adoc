...
---
= Limitations connues avec NetApp Backup and Recovery pour les charges de travail SQL Server
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Veuillez prendre connaissance de ces limitations avant d'utiliser NetApp Backup and Recovery pour les charges de travail SQL Server.



[[backup-and-recovery-preview-for-netapp-console-local-deployment-limitation]]
== Aperçu de la sauvegarde et de la restauration pour NetApp Console : limitation du déploiement local
...
Vous ne pouvez pas attribuer à une ressource plusieurs stratégies utilisant la même fréquence de planification (par exemple, horaire, quotidienne ou hebdomadaire). NetApp Backup and Recovery n’importera pas ces ressources depuis SnapCenter.
...
Si une politique SnapCenter se répète à la fois à des intervalles quotidiens et mensuels, NetApp Backup and Recovery n'importera pas la politique.

Par exemple, vous ne pouvez pas associer une politique quotidienne (avec un intervalle de 7 jours ou moins, ou de plus de 7 jours) à une ressource et également associer une politique mensuelle à la même ressource.
...
=== Les politiques de sauvegarde à la demande non migrées
...
SnapCenter ne mappe pas les clusters de stockage ni les machines virtuelles de stockage aux hôtes, mais NetApp Backup and Recovery le fait. Le cluster ONTAP sur site ou la machine virtuelle de stockage ne sera pas mappé à un hôte dans les versions préliminaires de NetApp Backup and Recovery. De plus, NetApp Console ne prend pas en charge les machines virtuelles de stockage.
