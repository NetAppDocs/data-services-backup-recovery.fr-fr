...
** Si vous créez des copies locales de Snapshots sur le volume de destination SnapMirror (quels que soient les libellés SnapMirror utilisés), ces Snapshots ne seront pas transférés vers le cloud en tant que sauvegardes. Vous devez créer une stratégie de Snapshot avec les libellés souhaités sur le volume DP source afin que NetApp Backup and Recovery puisse les sauvegarder.
