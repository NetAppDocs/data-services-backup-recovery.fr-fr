...
[role="lead"]
Veuillez prendre connaissance de ces limitations avant d'utiliser NetApp Backup and Recovery pour les volumes ONTAP.
...
* Vous ne pouvez sélectionner qu'un seul FlexGroup volume à la fois pour la réplication. Vous devez activer les sauvegardes séparément pour chaque FlexGroup volume.
+
Les volumes FlexVol ne présentent pas cette limitation : vous pouvez sélectionner tous les volumes FlexVol de votre système et leur attribuer les mêmes politiques de sauvegarde.
...
* Lorsque vous convertissez un volume FlexVol en volume FlexGroup ou inversement, le volume converti est considéré comme un nouveau volume ; vous devez configurer une protection pour le volume converti après la conversion. Vous pouvez restaurer les sauvegardes créées avant la conversion sur un volume différent.
...
* Ne sauvegardez pas les volumes de destination SnapMirror pour lesquels la conservation à long terme des snapshots est activée. Ces snapshots sont créés directement sur la destination et ne seront pas déplacés vers le stockage objet. Cette restriction s'applique même si la politique SnapMirror est ultérieurement modifiée pour désactiver la conservation à long terme.
...
+
** Les relations avec les labels SnapMirror `app_consistent` et  `all_source_snapshot` ne seront pas sauvegardées dans le cloud.
** Si vous créez des copies locales d'instantanés sur le volume de destination SnapMirror (quels que soient les libellés SnapMirror utilisés), ces instantanés ne seront pas transférés vers le cloud en tant que sauvegardes. Vous devez créer une règles de sauvegarde avec les libellés souhaités sur le volume DP source afin que NetApp Backup and Recovery puisse les sauvegarder.
...
** La stratégie de snapshot appliquée au volume doit être l'une des stratégies reconnues par NetApp Backup and Recovery, notamment quotidienne, hebdomadaire, mensuelle, etc. La stratégie par défaut « sm_created » (utilisée pour *Mirror All Snapshots*) n'est pas reconnue et le volume DP ne sera pas affiché dans la liste des volumes pouvant être sauvegardés.
** La sauvegarde et la restauration de volumes (SVM-DR) fonctionnent de manière totalement indépendante, que la sauvegarde provienne de la source ou de la destination. La seule restriction est que SVM-DR ne réplique pas la relation SnapMirror cloud. Dans le scénario de reprise après sinistre, lorsque la machine virtuelle de stockage est remise en ligne sur le site secondaire, vous devez mettre à jour manuellement la relation SnapMirror cloud.


* Prise en charge de MetroCluster :
+
** Lorsque vous utilisez ONTAP 9.12.1 GA ou une version ultérieure, la sauvegarde est prise en charge lors de la connexion au système principal. L'intégralité de la configuration de sauvegarde est transférée vers le système secondaire afin que les sauvegardes vers le cloud se poursuivent automatiquement après le basculement. Vous n'avez pas besoin de configurer la sauvegarde sur le système secondaire (en fait, cette opération vous est interdite).
...
** À partir de ONTAP 9.18.1, les sauvegardes de volumes FlexGroup sont prises en charge dans les configurations MetroCluster.


* La sauvegarde ad hoc de volumes à l'aide du bouton *Sauvegarder maintenant* n'est pas prise en charge sur les volumes de protection des données.
* Les configurations SM-BC ne sont pas prises en charge.
* ONTAP ne prend pas en charge la distribution des relations SnapMirror d'un seul volume vers plusieurs magasins d'objets ; par conséquent, cette configuration n'est pas prise en charge par NetApp Backup and Recovery.
* Le mode WORM/Conformité sur un stockage d'objets est pris en charge sur Amazon S3, Google Cloud Platform (GCP), Azure et StorageGRID actuellement. Ceci est connu sous le nom de fonctionnalité DataLock, et elle doit être gérée à l'aide des paramètres NetApp Backup and Recovery, et non via l'interface du fournisseur de cloud.
...
* La restauration des dossiers à partir d'instantanés locaux n'est pas prise en charge.
* Restauration de fichiers et de dossiers dans une configuration de reprise après sinistre (PRA) : après un basculement, les volumes d’une machine virtuelle de stockage configurée pour la PRA conservent des relations SnapMirror préexistantes dans un état dissocié. Les tentatives de restauration de fichiers ou de dossiers qui créent des relations SnapMirror RST temporaires sont bloquées par ONTAP car le volume de destination possède déjà une relation.
* Parcourir et restaurer peut restaurer jusqu'à 100 fichiers individuels à la fois.
* La fonction Rechercher et restaurer permet de restaurer un fichier à la fois.
...
== Activation de l'indexation dans une configuration de reprise après sinistre

Lors de l'activation de l'indexation sur un cluster de machines virtuelles de stockage dans une configuration de reprise après sinistre (DR), une erreur s'affiche indiquant que l'opération n'est pas autorisée sur un Vserver configuré comme destination pour le DR de Vserver. Malgré cette erreur, le processus backend définit le statut de l' `snapdiff-rpc-server`à "on" pour les machines virtuelles de stockage source. Après l'exécution de la planification de la stratégie de DR des machines virtuelles de stockage, ce statut est propagé aux machines virtuelles de stockage de destination.

*Solution de contournement* : Attendez que la planification de la stratégie de reprise après sinistre de la machine virtuelle de stockage s’exécute après l’erreur initiale, puis essayez de réactiver l’indexation.
