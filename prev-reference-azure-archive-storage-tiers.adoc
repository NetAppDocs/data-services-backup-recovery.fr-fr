...
[role="lead"]
Utilisez ces informations pour comprendre le niveau d’archivage Azure que NetApp Backup and Recovery prend en charge. Vous pouvez également consulter les coûts de restauration et les délais de récupération des sauvegardes archivées.
...
Lors de leur création, les fichiers de sauvegarde sont stockés dans le niveau d'accès _Cool_. Ce niveau est optimisé pour le stockage des données rarement consultées, mais il peut être accessible immédiatement en cas de besoin.
...
NOTE: La priorité élevée n'est pas prise en charge lors de la restauration de données d'Azure vers les systèmes StorageGRID.
