...
---
= Découvrez comment gérer les charges de travail Kubernetes avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
La gestion des charges de travail Kubernetes dans NetApp Backup and Recovery vous permet de découvrir, gérer et protéger vos clusters et applications Kubernetes depuis une interface unique. Vous pouvez gérer les ressources et les applications hébergées sur vos clusters Kubernetes. Vous pouvez également créer et associer des politiques de protection à vos applications Kubernetes, le tout via une interface unique.
