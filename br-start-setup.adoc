...
Après avoir configuré les options dans la destination de sauvegarde elle-même, vous la configurerez ultérieurement comme destination de sauvegarde dans NetApp Backup and Recovery.  Pour plus d'informations sur la configuration de la destination de sauvegarde dans NetApp Backup and Recovery, reportez-vous àlink:br-start-discover-backup-targets.html["Découvrir les cibles de sauvegarde"] .
