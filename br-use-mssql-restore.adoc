...
summary: 'Restaurez les charges de travail de la base de données à partir d"un instantané, d"une sauvegarde de charge de travail répliquée sur un stockage secondaire ou de sauvegardes stockées dans un stockage objet.' 
---
...
[role="lead"]
Restaurez les charges de travail Microsoft SQL Server à partir d'instantanés, de sauvegardes répliquées sur un stockage secondaire ou de sauvegardes dans un stockage objet. Choisissez le système d'origine, un autre système ou un système ONTAP local comme destination.

.Sources de restauration
Vous pouvez restaurer les charges de travail à partir des emplacements suivants :
...
Vous pouvez restaurer les données des manières suivantes :

* À partir d'un instantané
* À un moment précis où vous connaissez le nom du fichier, son emplacement et la dernière date de validité
* À la dernière sauvegarde


.Considérations relatives à la restauration à partir du stockage d'objets
Si vous sélectionnez un fichier de sauvegarde dans le stockage d'objets et que la protection contre les ransomwares est activée pour cette sauvegarde (si vous avez activé DataLock et la protection contre les ransomwares dans la règle de sauvegarde), vous êtes invité à effectuer une vérification d'intégrité supplémentaire sur le fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d'effectuer cette analyse.
...
== Comment fonctionne la restauration de la charge de travail
...
== Restaurez les données de charge de travail à partir de la page Restaurer

Utilisez la page Restaurer pour restaurer les charges de travail de la base de données.
...
+
Les options de destination s'affichent.
...
+
*** *Opérationnel* : Remet la base de données en ligne après l’application des sauvegardes du journal des transactions. Vous ne pouvez pas restaurer de journaux de transactions supplémentaires.
*** *Non opérationnel* : Maintient la base de données indisponible afin que vous puissiez restaurer des journaux de transactions supplémentaires.
*** *Mode lecture seule* : Restaure la base de données en mode lecture seule et vous permet de restaurer des journaux de transactions supplémentaires.
...
Cette option utilise les dernières sauvegardes complètes et les journaux pour restaurer les données à leur dernier bon état. NetApp Backup and Recovery analyse les journaux depuis le dernier instantané jusqu'à présent afin de restaurer la version la plus récente disponible des données.
...
. Sur la page *Restaurer le dernier état*, sélectionnez un emplacement d’instantané : stockage local, stockage secondaire ou stockage objet.
. Sélectionnez *Suivant*.
. Dans la page Détails de la destination, saisissez les informations suivantes :
+
** *Paramètres de destination* : Choisissez si vous souhaitez restaurer les données à leur emplacement d'origine ou à un autre emplacement.  Pour un autre emplacement, sélectionnez le nom de l’hôte et l’instance, entrez le nom de la base de données et entrez le chemin de destination.
...
== Restaurez les données de charge de travail à partir de la page Inventaire

Utilisez la page Inventaire pour restaurer les charges de travail de base de données. Vous pouvez restaurer les bases de données, mais pas les instances, à partir de cette page.
...
. Effectuez les étapes correspondant à l’option de restauration sélectionnée depuis la page Restauration.
