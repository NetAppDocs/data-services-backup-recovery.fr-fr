...
[role="lead"]
Surveillez les tâches de capture instantanée, de réplication, de sauvegarde et de restauration. Configurez les notifications par e-mail et utilisez la Console Timeline pour analyser l'activité.

NetApp Backup and Recovery conserve les informations relatives aux tâches pendant 15 jours, puis les supprime et les retire du Job Monitor.

*Rôle requis de la NetApp Console* : super administrateur Backup and Recovery, administrateur de sauvegarde Backup and Recovery, administrateur de restauration Backup and Recovery, administrateur de clonage Backup and Recovery ou rôle de visionneur Backup and Recovery. Si vous protégez des données stockées sur un cluster Cloud Volumes ONTAP, le rôle d’administrateur Cloud Volumes ONTAP est également requis. En savoir plus link:reference-roles.html["Rôles et privilèges de sauvegarde et de restauration"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["En savoir plus sur les rôles d'accès à la NetApp Console pour tous les services"^].
...
. Pour afficher des colonnes supplémentaires (Système, Storage VM, Nom d'utilisateur, Charge de travail, Nom de la stratégie, Étiquette de l'instantané), sélectionnez le signe plus.
...
+
Utilisez ce panneau pour effectuer une recherche en texte libre pour n'importe quelle ressource ; par exemple, « volume 1 » ou « application 3 ». Vous pouvez également filtrer la liste des tâches en fonction des éléments des menus déroulants.
...
[TIP]
====
** Vous pouvez rechercher des données dans une machine virtuelle de stockage spécifique uniquement si vous avez préalablement sélectionné un système.
...
Vous pouvez consulter les détails d'une tâche terminée. Vous pouvez exporter les détails d'une tâche particulière au format JSON.

Vous pouvez consulter des détails tels que le type de tâche (planifiée ou à la demande), le type de sauvegarde SnapMirror (initiale ou périodique), les heures de début et de fin, la durée, la quantité de données transférées du système vers le stockage objet, le taux de transfert moyen, le nom de la politique, le verrouillage de rétention activé, l’analyse de ransomware effectuée, les détails de la source de protection et les détails de la cible de protection.

Les tâches de restauration affichent des détails tels que le fournisseur cible de sauvegarde (Amazon Web Services, Microsoft Azure, Google Cloud, sur site), le nom du compartiment S3, le nom de la machine virtuelle de stockage, le nom du volume source, le volume de destination, l'étiquette de l'instantané, le nombre d'objets récupérés, les noms de fichiers, les tailles de fichiers, la date de dernière modification et le chemin d'accès complet au fichier.

.Étapes
. Dans le menu NetApp Backup and Recovery , sélectionnez *Surveillance*.
. Sélectionnez le nom du travail.
. Sélectionnez le menu Actions image:icon-action.png["Icône Actions"], puis *Afficher les détails*.
...
Surveillez les tâches de conservation (_cycle de vie des sauvegardes_) pour suivre l'expiration des copies de sauvegarde et prendre en charge les audits.
...
Le NetApp Console Notification Center suit la progression des tâches de sauvegarde et de restauration que vous avez lancées, ce qui vous permet de vérifier si l'opération a réussi.
...
| Opérations Cron | Incohérence entre l'étiquette de la règle d'instantané et celle de la règle de sauvegarde | Oui | Oui 


| Opérations Cron | Échec de la mise à jour des informations d'identification du magasin d'objets dans ONTAP pour ce système | Oui | Oui 
...
| Réplication | NetApp Échec de la tâche de pause de la relation de réplication Backup and Recovery | Oui | Non 


| Réplication | NetApp Échec de la tâche de rupture de la relation de réplication NetApp Backup and Recovery | Oui | Non 


| Réplication | NetApp Échec de la tâche de resynchronisation des relations de réplication NetApp Backup and Recovery | Oui | Non 


| Réplication | Échec de la tâche d'arrêt de la réplication de NetApp Backup and Recovery | Oui | Non 


| Réplication | NetApp Échec de la tâche de resynchronisation inverse de la relation de réplication NetApp Backup and Recovery | Oui | Oui 


| Réplication | NetApp échec de la tâche de suppression de la relation de réplication Backup and Recovery | Oui | Oui 
...
| Opérations système | L'indexation de la sauvegarde du volume a échoué | Oui | Oui 
|===
...
https://docs.netapp.com/us-en/console-local/task-alert-rules-manage.html["Configurez les règles de notification pour les alertes dans le déploiement local de la NetApp Console"^].
...
Vous pouvez consulter les détails des opérations de sauvegarde et de restauration pour une analyse plus approfondie dans la chronologie de la Console. La chronologie de la Console fournit des détails sur chaque événement, qu'il soit initié par l'utilisateur ou par le système, et affiche les actions initiées dans l'interface utilisateur ou via l'API.

https://docs.netapp.com/us-en/console-setup-admin/task-monitor-cm-operations.html["Découvrez comment surveiller les opérations de la NetApp Console"^].
