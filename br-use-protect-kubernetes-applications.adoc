...
---
= Ajoutez et protégez les applications Kubernetes dans NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez l’interface web NetApp Backup and Recovery ou des fichiers de ressources personnalisés pour ajouter des applications Kubernetes à protéger. Vous pouvez ajouter des applications basées sur un espace de noms utilisant des ressources Kubernetes standard ou des applications basées sur une ou plusieurs machines virtuelles.
...
Pour protéger une application Kubernetes, commencez par la créer dans NetApp Backup and Recovery. Lorsque vous créez l'application, NetApp Backup and Recovery prend connaissance de l'application en cours d'exécution sur le cluster Kubernetes.
...
. En haut à droite de la page, assurez-vous que *Kubernetes* est sélectionné dans la liste des charges de travail.
. Sélectionnez *Afficher* pour la ligne de charge de travail afin de voir les ressources Kubernetes.
...
. Dans la section *Stratégie*, vous pouvez choisir une stratégie de protection existante pour protéger l'application ou en créer une nouvelle. Si vous ne sélectionnez pas de stratégie, l'application est créée sans stratégie de protection. Vous pouvez link:br-use-policies-kubernetes-create.html#create-a-policy["ajouter une politique de protection"] ultérieurement.
...
NOTE: Si vous sélectionnez des machines virtuelles dans la liste, la définition de l'application est statique : aucune nouvelle machine virtuelle n'est ajoutée à l'application par la suite (vous devez modifier l'application pour les ajouter et les protéger). Si vous utilisez des sélecteurs par étiquette, vous ne pouvez pas sélectionner de machines virtuelles individuellement ni modifier la liste générée, mais toute machine virtuelle correspondant ultérieurement au sélecteur est automatiquement incluse et protégée.
...
.Résultat
L'application est créée et apparaît dans la liste des applications de l'onglet *Applications* de l'inventaire Kubernetes. NetApp Backup and Recovery permet de protéger l'application selon vos paramètres, et vous pouvez suivre la progression dans la section *Surveillance*.
