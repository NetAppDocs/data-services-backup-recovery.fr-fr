...
[role="lead"]
Dans NetApp Backup and Recovery, vous pouvez utiliser les règles de sauvegarde par défaut ou créer des règles personnalisées pour vos volumes ONTAP. Les règles contrôlent la fréquence des sauvegardes, leur planification et leur durée de conservation.
...
Lorsque vous utilisez l'assistant d'activation pour activer le service NetApp Backup and Recovery pour vos volumes, vous pouvez sélectionner parmi les règles de sauvegarde par défaut et toutes les autres règles de sauvegarde déjà présentes dans le système (Cloud Volumes ONTAP ou système ONTAP sur site). Si vous souhaitez utiliser une règle de sauvegarde différente de celles existantes, vous pouvez créer la règle avant ou pendant l'utilisation de l'assistant d'activation.
...
NetApp Backup and Recovery propose trois types de sauvegardes des données ONTAP : les instantanés, les réplications et les sauvegardes sur stockage objet. Leurs règles résident à différents emplacements selon l’architecture que vous utilisez et le type de sauvegarde :
...
. Dans la page Gestion des règles, sélectionnez *Créer une règle* > *Créer une règle d'instantané*.
...
.Étapes
. Dans la page Gestion des règles, sélectionnez *Créer une règle* > *Créer une règle de réplication*.
...
.Étapes
. Dans la page Gestion des politiques, sélectionnez *Créer une politique* > *Créer une règle de sauvegarde*.
. Dans la section Détails de la politique, spécifiez le nom de la politique.
. Spécifiez les étiquettes SnapMirror (maximum 5) indiquant la rétention pour chaque étiquette.
