...
[role="lead"]
Utilisez cette page pour vérifier que votre configuration est prise en charge et pour configurer les sauvegardes de vos systèmes ONTAP principaux sur site vers un stockage d'objets compatible S3 dans NetApp Backup and Recovery.
...
.Étapes
. Accédez à l'assistant d'activation de la sauvegarde et de la restauration en utilisant l'une des méthodes suivantes :
+
** Dans la page *Systems* de la NetApp Console, sélectionnez le système et sélectionnez *Enable > Backup Volumes* à côté de Backup and Recovery dans le panneau de droite.
...
====
.Étapes
Notez que si des règles de snapshot ou de réplication sont déjà appliquées aux volumes que vous choisissez, les règles que vous sélectionnerez ultérieurement écraseront ces règles existantes.
...
TIP: Si vous souhaitez créer une stratégie personnalisée avant d’activer l’instantané, vous pouvez utiliser System Manager ou la commande ONTAP CLI `snapmirror policy create`. Consultez.
...
** *Exporter les snapshots existants vers le stockage objet en tant que fichiers de sauvegarde* : Si des snapshots locaux pour les volumes de ce système correspondent à l'étiquette de planification de sauvegarde que vous venez de sélectionner (par exemple, quotidienne, hebdomadaire, etc.), cette invite supplémentaire s'affiche. Cochez cette case pour que tous les snapshots historiques soient copiés vers le stockage objet en tant que fichiers de sauvegarde afin d'assurer la protection la plus complète de vos volumes.
...
. Cochez éventuellement la case pour *synchroniser automatiquement les étiquettes de la stratégie d'instantané avec les étiquettes des stratégies de réplication et des règles de sauvegarde*. Cela crée des instantanés avec une étiquette correspondant à celles des stratégies de réplication et des règles de sauvegarde. Si les stratégies ne correspondent pas, les sauvegardes ne seront pas créées.
...
Vous pouvez afficher et, éventuellement, copier les commandes API utilisées dans l'assistant d'activation de NetApp Backup and Recovery. Vous pouvez faire cela pour automatiser l'activation de la sauvegarde dans les systèmes futurs.

.Étapes
. Dans l'assistant Activer la sauvegarde et la restauration, sélectionnez *Afficher la requête API*.
