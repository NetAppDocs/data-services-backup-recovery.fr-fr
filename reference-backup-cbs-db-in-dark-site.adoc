...
summary: 'En mode privé (sans accès Internet), NetApp Backup and Recovery stocke ses données de configuration dans le même compartiment StorageGRID ou ONTAP S3 que celui utilisé pour vos sauvegardes. En cas de défaillance du système hôte de l’agent Console, vous pouvez déployer un nouvel agent Console et restaurer ces données.' 
---
...
[role="lead"]
En mode privé (sans accès Internet), NetApp Backup and Recovery stocke ses données de configuration dans le même compartiment StorageGRID ou ONTAP S3 utilisé pour vos sauvegardes. En cas de défaillance du système hôte de l’agent Console, vous pouvez déployer un nouvel agent Console et restaurer ces données.

Vous pouvez également suivre ces étapes pour remplacer l’agent de la Console et restaurer les données de configuration qui y étaient stockées lors de l’utilisation de la sauvegarde et de la restauration en mode standard sur site, lorsqu’aucun compte cloud n’est impliqué (par exemple, lors de l’utilisation de NetApp StorageGRID comme destination de sauvegarde).
...
Lors de l'installation d'un nouvel agent Console, téléchargez la même version logicielle que l'agent d'origine. Les modifications apportées à la base de données NetApp Backup and Recovery peuvent empêcher les nouvelles versions logicielles de fonctionner avec les anciennes sauvegardes de la base de données. Vous pouvez https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html["mettre à niveau le logiciel de l'agent de la console vers la version la plus récente après la restauration de la base de données de sauvegarde"^].
...
. Connectez-vous à la NetApp Console en utilisant les identifiants de l’utilisateur administrateur que vous venez de créer.
...
. Copiez la sauvegarde MySQL depuis son emplacement de sauvegarde vers le nouvel hôte de l'agent Console. L'exemple suivant utilise le nom de fichier `CBS_DB_Backup_23_05_2023.sql`.
...
----
. Vérifiez que les volumes affichés correspondent bien à ceux de votre environnement d'origine.
...
. Copiez le fichier ZIP de sauvegarde du catalogue indexé depuis l'emplacement de sauvegarde vers le dossier  `/opt/application/netapp/cbs` sur le nouvel hôte de l'agent Console. L'exemple suivant utilise le nom de fichier  `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`.
...
----
. Exécutez la commande *ls* pour vous assurer que le dossier « catalogdb1 » a été créé et qu'il contient les sous-dossiers `files`, `versions` et `snapshots`.
...
Ajoutez les détails du système StorageGRID associé à vos systèmes ONTAP, tels qu'ils ont été configurés dans la configuration d'origine de l'agent Console, en utilisant  https://docs.netapp.com/us-en/console-automation/index.html["API de la NetApp Console"^].
...
----
+
L'adresse IP, le nom d'utilisateur et le mot de passe sont des valeurs personnalisées, contrairement au nom du compte. Le nom du compte est toujours  `account-DARKSITE1`. De plus, le nom d'utilisateur doit utiliser un nom au format adresse électronique.

+
Cette API renvoie une réponse similaire à celle-ci. Vous pouvez récupérer le jeton d'autorisation comme indiqué ci-dessous.
...
----
+
Cette API renvoie une réponse similaire à la suivante. La valeur sous  `resourceIdentifier` désigne le _WorkingEnvironment ID_, et la valeur sous  `agentId` désigne _x-agent-id_.
...
----
. Mettez à jour la base de données NetApp Backup and Recovery avec les informations du système StorageGRID associé aux systèmes. Assurez-vous de saisir le domaine complet de StorageGRID, ainsi que la clé d'accès et la clé de stockage, comme indiqué ci-dessous :
+
[source, shell]
----
...
. Sélectionnez chaque système ONTAP et sélectionnez *Afficher les sauvegardes* à côté du service Backup and Recovery dans le panneau de droite.
+
Vous devriez voir toutes les sauvegardes créées pour vos volumes.

. Dans le tableau de bord de restauration, sous la section Recherche & Restauration, sélectionnez *Paramètres d'indexation*.
+
Assurez-vous que les systèmes dont le catalogage indexé était activé précédemment restent activés.
