...
---
= Découvrez la protection des volumes ONTAP avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez NetApp Backup and Recovery pour protéger et restaurer les données de votre volume ONTAP et pour archiver les sauvegardes à long terme. Vous pouvez mettre en œuvre une stratégie 3-2-1 avec trois copies de vos données sources sur deux systèmes de stockage différents et une copie dans le cloud.
...
Après activation, NetApp Backup and Recovery crée des sauvegardes incrémentales permanentes au niveau bloc, stockées sur un autre cluster ONTAP et dans un stockage objet dans le cloud. Outre votre volume source, vous disposerez de :
...
image:diagram-321-overview-unified.png["Un diagramme montrant comment les fichiers de sauvegarde existent sur le système source sous forme d'instantanés, sous forme de volumes répliqués sur le système de stockage secondaire et sous forme de fichiers de sauvegarde dans le stockage objet."]

NetApp Backup and Recovery utilise la technologie de réplication des données SnapMirror de NetApp pour maintenir vos sauvegardes synchronisées en créant des snapshots et en les transférant vers les emplacements de sauvegarde.
...
* Configurez le chiffrement natif en transit d'ONTAP en utilisant une clé pré-partagée (PSK) entre les deux systèmes.
...
* Comparée à  https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^], la réplication dans NetApp Backup and Recovery inclut les fonctionnalités suivantes :
+
...
* Analysez les fichiers de sauvegarde pour détecter d'éventuelles attaques de ransomware et supprimez ou remplacez automatiquement les sauvegardes infectées.
...
https://bluexp.netapp.com/cloud-volumes-global-regions["En savoir plus en utilisant la carte des régions mondiales"^]
...
NetApp Backup and Recovery vous permet de sauvegarder des volumes ONTAP à partir des systèmes sources suivants vers les systèmes secondaires et le stockage d'objets suivants, chez des fournisseurs de cloud public et privé. Les snapshots résident sur le système source.


NOTE: Wasabi est actuellement le seul fournisseur de stockage objet compatible S3 qualifié. Les autres fournisseurs compatibles S3 n'ont pas été validés.
...
| Cloud Volumes ONTAP dans AWS  a| 
...
| Cloud Volumes ONTAP dans Azure  a| 
...
| Cloud Volumes ONTAP dans Google  a| 
...
| Stockage d'objets compatible S3 | Système ONTAP sur site | Système ONTAP sur site 
|===

NOTE: Les références à « on-premises ONTAP system » incluent les systèmes FAS, AFF et ONTAP Select.
...
* Si vous prévoyez d'analyser un fichier de sauvegarde à la recherche de ransomware lors du processus de restauration des données du volume (si vous avez activé DataLock et la protection contre les ransomwares pour vos sauvegardes cloud), alors vous devrez également payer des frais de sortie supplémentaires à votre fournisseur de cloud.
...
* Les _instantanés_ résident sur le volume source du système source.
...
+
** Dans AWS, la console permet https://docs.aws.amazon.com/AmazonS3/latest/dev/access-control-block-public-access.html["Fonctionnalité d'accès public au bloc Amazon S3"^] sur le compartiment S3.
...
NOTE: La stratégie de snapshot appliquée au volume doit comporter l'une des étiquettes que vous utilisez dans votre stratégie de réplication et votre stratégie de sauvegarde sur objet. Si aucune étiquette correspondante n'est trouvée, aucun fichier de sauvegarde ne sera créé. Par exemple, si vous souhaitez créer des volumes répliqués et des fichiers de sauvegarde « weekly », vous devez utiliser une stratégie de snapshot qui crée des snapshots « weekly ».
...
Si votre cluster utilise ONTAP 9.11.1 ou une version ultérieure, vous pouvez protéger vos sauvegardes dans le stockage objet contre la suppression et les attaques par rançongiciel. Chaque règle de sauvegarde comprend une section pour _DataLock and Ransomware protection_ qui peut être appliquée à vos fichiers de sauvegarde pendant une période spécifique : la _période de rétention_.
...
La prise en charge est actuellement disponible lorsque votre destination de sauvegarde est Amazon S3, Azure Blob ou StorageGRID. D'autres destinations de fournisseur de stockage seront ajoutées dans les prochaines versions.
...
+
Si votre cluster local utilise ONTAP 9.12.1 ou une version ultérieure, et que votre système StorageGRID utilise la version 11.4 ou une version ultérieure, vous pouvez archiver les fichiers de sauvegarde plus anciens vers un stockage d’archivage cloud public après un certain nombre de jours. La prise en charge actuelle concerne les niveaux de stockage AWS S3 Glacier/S3 Glacier Deep Archive ou Azure Archive. link:prev-ontap-backup-onprem-storagegrid.html["En savoir plus sur l'archivage des fichiers de sauvegarde depuis StorageGRID"].



Voir link:prev-ontap-policy-object-options.html["Comment fonctionne l'archivage des anciens fichiers de sauvegarde"].
