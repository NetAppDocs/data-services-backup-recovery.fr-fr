...
---
= Différences de politique entre SnapCenter et NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Utilisez ce document de référence pour comprendre les différences de règles entre SnapCenter et NetApp Backup and Recovery qui peuvent affecter les ressources et les règles importées.
...
NetApp Backup and Recovery utilise les niveaux de planification suivants, qui diffèrent légèrement :
...
NetApp Backup and Recovery ajuste les calendriers SnapCenter comme suit :
...
* Si la planification SnapCenter est supérieure à 7 jours, NetApp Backup and Recovery règle la planification sur une base mensuelle. Certaines snapshots seront ignorées durant le mois.
+
...
*Exemple* : Si vous avez une politique SnapCenter horaire avec un intervalle de répétition toutes les 5 heures à partir de 1 h du matin, NetApp Backup and Recovery (après la migration) configurera la planification sur une base quotidienne avec des intervalles de 5 heures à 1 h, 6 h, 11 h, 16 h et 21 h. Certaines heures seront ignorées : après 21 h, cela devrait être 2 h du matin pour répéter toutes les 5 heures, mais ce sera toujours à 1 h du matin.
