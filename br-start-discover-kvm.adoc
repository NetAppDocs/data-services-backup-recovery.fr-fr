...
.. *Paramètres KVM* : Procédez comme suit :
+
... Si vous n'avez pas encore enregistré d'identifiants pour cet hôte KVM, sélectionnez *Ajouter des identifiants*.
+
...
... Ajoutez un nouvel hôte KVM en saisissant les informations suivantes :
+
**** *Nom de domaine complet KVM ou adresse IP* : Saisissez le nom de domaine complet ou l’adresse IP de l’hôte.
**** *Agents* : Choisissez l’agent Console à utiliser pour la communication entre NetApp Backup and Recovery et l’hôte KVM.
**** *Identifiants* : Sélectionnez un identifiant stocké pour l’hôte KVM.




.. *Paramètres avancés* : Saisissez éventuellement un port pour communiquer avec l’hôte KVM et sélectionnez la plateforme de gestion de l’hôte dans la liste.
