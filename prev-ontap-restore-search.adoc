...
[role="lead"]
Vous pouvez utiliser la fonction Search and Restore pour récupérer des volumes, des dossiers ou des fichiers à partir de fichiers de sauvegarde ONTAP. Search and Restore vous permet d’effectuer des recherches dans toutes les sauvegardes (y compris les instantanés locaux, les volumes répliqués et le stockage objet) sans avoir besoin de connaître précisément le nom du système, du volume ou du fichier.
