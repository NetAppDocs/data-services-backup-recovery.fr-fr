...
---
= Restaurez les données ONTAP à l'aide de la fonction Rechercher et restaurer dans NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez la fonction Search and Restore pour récupérer des volumes, des dossiers ou des fichiers à partir de sauvegardes ONTAP lorsque vous ne connaissez pas le nom exact du système, du volume ou du fichier. Vous pouvez effectuer une recherche dans les instantanés locaux, les volumes répliqués et les sauvegardes de stockage objet.
...
* Lorsque vous restaurez un volume activé SnapLock, le volume restauré conserve la protection SnapLock uniquement si le stockage de destination est compatible WORM. Si vous restaurez vers une destination non compatible WORM, le statut SnapLock est supprimé du volume restauré.


====
Avant de commencer, vous devriez avoir une idée du nom ou de l'emplacement du volume ou du fichier que vous souhaitez restaurer.
...
| StorageGRID | Système ONTAP sur site Cloud Volumes ONTAP | Système ONTAP sur site 
...
** Vous devez configurer une interface logique de données NFS IPv4 sur la machine virtuelle de stockage et une exportation NFS pour le volume. Search & Restore prend en charge les volumes NFS et SMB/CIFS.
** Le serveur RPC SnapDiff doit être activé sur la machine virtuelle de stockage. La Console effectue cette opération automatiquement lorsque vous activez l’indexation sur le système. (SnapDiff est la technologie qui identifie rapidement les différences de fichiers et de répertoires entre les instantanés.)
...
NOTE: La fonction de sauvegarde et de récupération n'indexe pas les instantanés dont l'horodatage de création correspond exactement à un autre instantané du même volume.
...
.Étapes pour activer l'indexation d'un système :
Si l’indexation a déjà été activée pour votre système, passez à la section suivante pour restaurer vos données.

Commencez par monter un volume distinct pour stocker les fichiers de catalogue. Cela permet d'éviter une perte de données si les fichiers de catalogue des instantanés deviennent trop volumineux. Cette étape n'est pas nécessaire sur chaque cluster ; vous pouvez monter n'importe quel volume provenant de l'un des clusters de votre environnement. Si vous ne le faites pas, l'indexation pourrait ne pas fonctionner correctement.
...
[[mount-volume-steps]]
.Étape pour monter le volume en vue de la réindexation du catalogue
...
.. Dans la section « Filtrer par », sélectionnez les critères de filtrage. Par exemple, vous pouvez sélectionner le système où résident les données et le type de fichier, tel qu’un fichier .JPEG. Vous pouvez également sélectionner le type d’emplacement de sauvegarde si vous souhaitez rechercher uniquement parmi les instantanés ou les fichiers de sauvegarde disponibles dans le stockage objet.
...
+
Notez que les résultats identifient les instantanés de volumes locaux et les volumes répliqués distants contenant le fichier dans votre recherche. Vous pouvez choisir de restaurer à partir du fichier de sauvegarde cloud, de l'instantané ou du volume répliqué.
...
+
*** Lors de la restauration à partir d'Amazon S3, sélectionnez l'IPspace dans le cluster ONTAP où résidera le volume de destination, saisissez la clé d'accès et la clé secrète de l'utilisateur que vous avez créé pour donner au cluster ONTAP l'accès au compartiment S3, et choisissez éventuellement un point de terminaison VPC privé pour un transfert de données sécurisé. Si vous utilisez un point de terminaison VPC privé, il est prioritaire sur toute connexion proxy configurée. link:prev-ontap-backup-onprem-aws.html["Voir les détails sur ces exigences"].
