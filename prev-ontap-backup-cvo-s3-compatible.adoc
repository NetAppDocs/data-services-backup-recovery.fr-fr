...
---
= Sauvegardez les données Cloud Volumes ONTAP sur un stockage compatible S3 avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Suivez quelques étapes dans NetApp Backup and Recovery pour commencer à sauvegarder les données de volume de vos systèmes Cloud Volumes ONTAP vers S3.
...
Informations requises pour l'utilisation de clés gérées par le client pour le chiffrement des données:: Vous pouvez choisir vos propres clés gérées par le client pour le chiffrement des données dans l'assistant d'activation au lieu d'utiliser les clés de chiffrement S3 par défaut. Dans ce cas, vous devrez avoir déjà configuré les clés de chiffrement gérées. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Découvrez comment utiliser vos propres clés"^]
...
Autorisations AWS Cloud Volumes ONTAP requises:: Lorsque votre système Cloud Volumes ONTAP exécute le logiciel ONTAP 9.12.1 ou une version supérieure, le rôle IAM qui fournit à ce système les autorisations doit inclure un nouvel ensemble d'autorisations compatibles S3 spécifiquement pour NetApp Backup and Recovery à partir de la dernière https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Politique Cloud Volumes ONTAP"^].
+
--
...
+
Si la destination compatible S3 pour vos sauvegardes existe sous forme de cluster sur la page *Systèmes*, vous pouvez faire glisser le cluster sur le système compatible S3 pour lancer l'assistant d'installation.
...
** Sélectionnez *Volumes* dans la barre Sauvegarde et restauration. Dans l'onglet Volumes, sélectionnez *Actions* image:icon-action.png["Icône Actions"] Choisissez l'option icône et sélectionnez *Activer la protection 3-2-1* pour un seul volume (qui n'a pas déjà la réplication ou la sauvegarde sur le stockage d'objets activée).
...
. *Sauvegarde* : Définissez les options suivantes :
+
...
. Cochez éventuellement la case pour *corriger automatiquement les étiquettes incorrectes sur les instantanés locaux, la réplication et la sauvegarde*. Cela crée des instantanés avec une étiquette qui correspond aux étiquettes des politiques d'instantané, de réplication et de sauvegarde.
