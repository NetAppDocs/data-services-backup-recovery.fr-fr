...
[role="lead"]
Utilisez cette présentation pour découvrir comment NetApp Backup and Recovery protège vos charges de travail ONTAP, quelles charges de travail et cibles de sauvegarde il prend en charge, et comment fonctionnent les licences et les coûts.
...
La prise en charge de la sauvegarde et de la restauration est déjà intégrée à tous les systèmes ONTAP, il n’est donc pas nécessaire d’ajouter du matériel ou des passerelles de média supplémentaires. Cela rend les opérations de sauvegarde simples et rentables. NetApp Console simplifie la mise en œuvre de toute stratégie de sauvegarde, y compris l’ensemble des variantes de sauvegarde 3-2-1, sans avoir besoin de plusieurs gestionnaires de ressources ou de personnel spécialisé.
...
** Sauvegardez les charges de travail VMware sur des fournisseurs de stockage compatibles.
...
** Se référer à link:br-use-oracle-protect-overview.html["Protégez les charges de travail Oracle Database"].
...
** Dans Google Cloud, un nouveau bucket est déployé, et les  https://cloud.google.com/bigquery["Services Google Cloud BigQuery"^] sont provisionnées au niveau du compte/projet.
...
* Si vous prévoyez d'analyser un fichier de sauvegarde à la recherche de ransomware lors du processus de restauration des données du volume (si vous avez activé Datalock et la protection Ransomware pour vos sauvegardes cloud), alors vous devrez également payer des frais de sortie supplémentaires auprès de votre fournisseur de cloud.
...
* La troisième option consiste à acheter des licences directement auprès de NetApp. Consultez la section <<Licences>> pour plus de détails.
...
* Stockage Google Cloud (volumes ONTAP et charges de travail Kubernetes uniquement)
* StorageGRID
* ONTAP S3
* Stockage compatible S3 (charges de travail ONTAP Volumes uniquement)
...
* Dans Azure, NetApp Console utilise un groupe de ressources nouveau ou existant avec un compte de stockage pour le conteneur Blob. La Console https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["bloque l'accès public à vos données blob"] par défaut.
...
== Conditions générales de NetApp Backup and Recovery
