...
** *Cascade 3-2-1* : (Non disponible pour les charges de travail Kubernetes) Stockage principal (disque) vers stockage secondaire (disque) et stockage principal (disque) vers stockage cloud (stockage objet). Il peut s'agir d'un stockage objet d'un hyperscaler cloud ou d'un stockage objet privé tel que StorageGRID. Cela crée une chaîne de réplication des données sur plusieurs systèmes afin de garantir la redondance et la fiabilité. Cette option n'est pas disponible pour Amazon FSx for NetApp ONTAP.
+
...
** *Distribution disque à disque* : (Non disponible pour les charges de travail Kubernetes) Stockage principal (disque) vers stockage secondaire (disque) et stockage principal (disque) vers stockage secondaire (disque). Vous pouvez configurer plusieurs paramètres secondaires pour l’option de distribution disque à disque.
+
...
** *Fournisseur* : (charges de travail Kubernetes uniquement) Sélectionnez le fournisseur de stockage qui héberge les ressources de l’application Kubernetes et saisissez les identifiants pour vous authentifier auprès du fournisseur.
...
** *Activer l'analyse d'intégrité* : (Non disponible pour les charges de travail Kubernetes) Choisissez d'activer ou non les analyses d'intégrité (verrouillage des snapshots) sur le stockage d'objets. Cela garantit que les sauvegardes sont valides et restaurables. La fréquence des analyses d'intégrité est définie à 7 jours par défaut. Pour protéger vos sauvegardes contre toute modification ou suppression, sélectionnez l'option *Analyse d'intégrité*. L'analyse est effectuée uniquement sur le dernier snapshot. Vous pouvez activer ou désactiver les analyses d'intégrité sur le dernier snapshot.
...
** *Tentatives de sauvegarde* : (Non applicable aux charges de travail VMware) Pour relancer la tâche en cas d’échec ou d’interruption, sélectionnez *Activer les nouvelles tentatives en cas d’échec*. Indiquez le nombre maximal de tentatives pour les tâches de snapshot et de sauvegarde, ainsi que l’intervalle de temps entre les tentatives. Le nombre de tentatives doit être inférieur à 10.
+
