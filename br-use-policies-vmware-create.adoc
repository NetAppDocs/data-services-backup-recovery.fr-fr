...
[role="lead"]
Dans NetApp Backup and Recovery, créez vos propres stratégies de sauvegarde VMware qui régissent la fréquence des sauvegardes, la durée de sauvegarde et le nombre de fichiers de sauvegarde conservés.
...
+
*** *Activer les snapshots cohérents avec la VM et cohérent au niveau des applications* : Activez cette option pour créer des snapshots cohérents avec la VM et cohérent au niveau des applications. VMware Tools doit être en cours d’exécution sur la VM. Si VMware Tools n’est pas en cours d’exécution, les snapshots seront cohérents en cas de panne. Notez que l’activation de cette option peut augmenter la durée de sauvegarde et utiliser plus d’espace de stockage. De plus, la RAM active de la VM n’est pas incluse dans les snapshots cohérents.
