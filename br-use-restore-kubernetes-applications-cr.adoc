...
---
= Restaurez des applications Kubernetes avec des ressources personnalisées
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez des ressources personnalisées pour restaurer vos applications à partir d'un instantané ou d'une sauvegarde. La restauration à partir d'un instantané existant est généralement plus rapide si vous restaurez l'application sur le même cluster.
...
* Vous pouvez utiliser les paramètres avancés pour personnaliser les opérations de restauration. Pour en savoir plus, consultez link:br-use-kubernetes-advanced-restore-settings.html["Paramètres de restauration avancés pour Kubernetes dans NetApp Backup and Recovery"].
...
* Lorsque vous utilisez un CR pour restaurer dans un nouvel espace de noms, vous devez créer manuellement l'espace de noms de destination avant d'appliquer le CR. Backup and Recovery crée automatiquement les espaces de noms uniquement lorsque vous utilisez la CLI.
...
NOTE: Si le BackupRestore CR n'est pas disponible, vous pouvez utiliser la commande de l'étape 2 pour afficher le contenu de la sauvegarde.
