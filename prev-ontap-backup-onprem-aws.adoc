...
[role="lead"]
Dans NetApp Backup and Recovery, vous pouvez sauvegarder les données de volumes de vos systèmes ONTAP locaux vers un système de stockage secondaire et vers Amazon S3. Utilisez cette page pour préparer votre environnement et activer les sauvegardes.
...
NOTE: Le chiffrement avec clé gérée par le client est pris en charge uniquement pour Amazon FSx for NetApp ONTAP. Il n'est pas pris en charge pour Cloud Volumes ONTAP for AWS.

Consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Configurer le chiffrement géré par le client dans AWS"^].
