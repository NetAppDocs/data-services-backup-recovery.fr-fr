...
---
= Sauvegardez Cloud Volumes ONTAP sur le stockage Blob Azure avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez NetApp Backup and Recovery pour sauvegarder les données de volume de vos systèmes Cloud Volumes ONTAP vers le stockage Blob Azure.
...
Vous avez également besoin d'un abonnement Azure pour l'espace de stockage où vos sauvegardes seront situées.
...
Pour utiliser la fonctionnalité de recherche et de restauration NetApp Backup and Recovery, le rôle de l’agent de la Console doit inclure des autorisations spécifiques afin de pouvoir accéder à l’espace de travail Azure Synapse et au compte Azure Data Lake Storage. Consultez les autorisations ci-dessous et suivez les étapes si vous devez modifier la politique.
...
Choisissez les volumes que vous souhaitez protéger. Un volume protégé est un volume qui possède une ou plusieurs des règles suivantes : règles de snapshot, règles de réplication, règles de sauvegarde sur objet.
...
NOTE: Si vous avez choisi un compte de stockage Azure existant, les informations de chiffrement sont déjà disponibles, vous n’avez donc pas besoin de les saisir maintenant.
