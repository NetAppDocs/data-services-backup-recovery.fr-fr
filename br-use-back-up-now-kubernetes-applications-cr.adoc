...
summary: 'Utilisez des ressources personnalisées (CR) pour créer des instantanés et des sauvegardes à la demande d"une application Kubernetes, établissant une base de référence ou protégeant vos modifications de données les plus récentes.' 
---
= Sauvegardez dès maintenant des applications Kubernetes à l'aide de ressources personnalisées dans NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez des ressources personnalisées (CR) pour créer des instantanés et des sauvegardes à la demande d'une application Kubernetes, établissant une base de référence ou protégeant vos modifications de données les plus récentes.
...
** *spec.reclaimPolicy* : (_Optionnel_) Définit ce qui arrive à l'AppArchive d'un instantané lorsque la ressource personnalisée Snapshot est supprimée. Même lorsque cette option est définie sur  `Retain`, la ressource personnalisée Snapshot elle-même est toujours supprimée ; seule l'AppArchive est conservée. Options valides :
+
*** `Retain` (défaut)
*** `Delete`
+
...
** *spec.dataMover* : (_Optionnel, requis pour les clusters migrés depuis Trident Protect_) Une chaîne indiquant l’outil de sauvegarde à utiliser pour l’opération de sauvegarde. Si ce cluster a été migré vers Backup and Recovery depuis Trident Protect, la valeur est sensible à la casse et doit être `CBS`.
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | chaîne | Temps maximal (en secondes) à attendre pour que tout nouveau PersistentVolumeClaims (PVC) atteigne la  `Bound` phase avant que l’opération n’échoue. | "1200" (20 minutes) 
|===