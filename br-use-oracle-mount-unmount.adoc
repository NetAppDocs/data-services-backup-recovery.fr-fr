...
---
= Montez, démontez et cataloguez les points de récupération de la base de données Oracle avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Il peut être judicieux de monter un point de récupération Oracle Database si vous devez accéder à la base de données dans un état contrôlé pour effectuer des opérations de restauration. Cataloguez manuellement le point de récupération si vous devez stocker les métadonnées de sauvegarde et de restauration dans le catalogue de récupération RMAN et que l'option de catalogage automatique des métadonnées n'est pas activée.



== Monter un point de récupération de base de données Oracle
...
== Démonter un point de récupération de base de données Oracle
...
== Cataloguez un point de récupération de la base de données Oracle

Cataloguez manuellement un point de récupération lorsque vous devez préserver les métadonnées de ses opérations de sauvegarde et de restauration. Les métadonnées sont stockées conformément aux paramètres de *Configure RMAN catalog* que vous choisissez pour la base de données (dans le fichier de contrôle cible par défaut). Vous n'avez besoin de cataloguer manuellement que si le catalogage automatique est désactivé dans la stratégie de protection associée.
...
. Choisissez un point de récupération dans la liste et sélectionnez l'icône Actions image:../media/icon-action.png["Option Actions"] > *Catalogue*.
. Confirmez l'action en sélectionnant *Catalog*.
