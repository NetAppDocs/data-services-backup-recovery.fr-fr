...
Les actions et configurations suivantes ne sont pas prises en charge dans les charges de travail KVM dans NetApp Backup and Recovery :
...
Veuillez prendre note des points suivants lors de l'utilisation de charges de travail KVM avec NetApp Backup and Recovery.

* Les sauvegardes peuvent échouer si l'agent invité QEMU n'est pas exécuté sur une machine virtuelle protégée. Pour contourner ce problème, assurez-vous que le  `qemu-guest-agent` daemon est en cours d'exécution sur la machine virtuelle avant de lancer la sauvegarde.
