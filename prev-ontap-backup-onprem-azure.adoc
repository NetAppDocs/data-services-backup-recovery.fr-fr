...
[role="lead"]
Dans NetApp Backup and Recovery, vous pouvez sauvegarder les données de volumes de vos systèmes ONTAP locaux vers un système de stockage secondaire et vers Azure Blob. Utilisez cette page pour préparer votre environnement et activer les sauvegardes.
...
+
NetApp Backup and Recovery est pris en charge dans les régions Azure Government lorsque l'agent Console est déployé dans le cloud, et non lorsqu'il est installé sur vos locaux. De plus, vous devez déployer l'agent Console depuis Azure Marketplace. Vous ne pouvez pas déployer l'agent Console dans une région Government depuis le site web NetApp Console SaaS.
...
+
** Pour la licence PAYGO de NetApp Backup and Recovery, vous aurez besoin d'un abonnement à la  https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Offre NetApp Console de la Place de marché Azure"^]. La facturation de NetApp Backup and Recovery s'effectue via cet abonnement.
...
* Si vous utilisez un IPspace différent de celui par défaut, vous devrez peut-être créer une route statique pour accéder au stockage d'objets.
...
+
Notez que NetApp Backup and Recovery prend en charge les stratégies d’accès Azure comme modèle d’autorisation. Le modèle d’autorisation _Azure role-based access control_ (Azure RBAC) n’est actuellement pas pris en charge.
...
+
** Dans la page *Systems* de la NetApp Console, sélectionnez le système et sélectionnez *Enable > Backup Volumes* à côté du service NetApp Backup and Recovery dans le panneau de droite.
+
...
+
La page d'introduction de l'assistant présente les options de protection, notamment les instantanés locaux, la réplication et les sauvegardes. Si vous avez choisi la deuxième option à cette étape, la page « Définir la stratégie de sauvegarde » s'affiche avec un volume sélectionné.
...
NOTE: Si vous sélectionnez *Bring your own bucket*, assurez-vous que le conteneur que vous créez comme destination de sauvegarde porte le même nom que le compte de stockage Azure.
