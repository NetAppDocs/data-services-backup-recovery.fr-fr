...
Avant la restauration, notez le nom du volume source, le système source et le SVM, ainsi que la date du fichier de sauvegarde. Vous pouvez restaurer les données ONTAP à partir d'un instantané, d'un volume répliqué ou de sauvegardes stockées dans un stockage objet.


NOTE: Si le fichier de sauvegarde que vous souhaitez restaurer se trouve dans un stockage cloud d'archivage (à partir d' ONTAP 9.10.1), l'opération de restauration prend plus de temps et engendre des coûts.  Le cluster de destination doit également exécuter ONTAP 9.10.1 ou supérieur pour la restauration de volumes, 9.11.1 pour la restauration de fichiers, 9.12.1 pour Google Archive et StorageGRID, et 9.13.1 pour la restauration de dossiers.
...
. Pour restaurer un dossier ou des fichiers à partir d'une sauvegarde de volume, sélectionnez le menu *Restaurer*, puis *Restaurer des fichiers ou un dossier* sous _Parcourir et restaurer_.
...
L'opération de recherche examine tous les instantanés locaux existants pour vos volumes ONTAP , tous les volumes répliqués sur les systèmes de stockage secondaires et tous les fichiers de sauvegarde existants dans le stockage d'objets.  Étant donné que la restauration de données à partir d'un instantané local ou d'un volume répliqué peut être plus rapide et moins coûteuse que la restauration à partir d'un fichier de sauvegarde dans un stockage objet, vous pourriez souhaiter restaurer les données à partir de ces autres emplacements.
