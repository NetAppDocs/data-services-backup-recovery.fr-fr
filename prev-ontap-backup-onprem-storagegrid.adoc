...
Pour plus de détails sur les exigences de Datalock et de protection contre les ransomwares pour StorageGRID, veuillez vous référer à link:prev-ontap-policy-object-options.html["Options de politique de sauvegarde sur objet"].
...
+
Si votre cluster utilise ONTAP 9.11.1 ou une version ultérieure, vous pouvez choisir de protéger vos sauvegardes contre la suppression et les attaques de ransomware en configurant _DataLock_ et _Ransomware protection_. _DataLock_ protège vos fichiers de sauvegarde contre toute modification ou suppression, et _ransomware protection_ analyse vos fichiers de sauvegarde pour rechercher des preuves d'une attaque de ransomware dans vos fichiers de sauvegarde.
