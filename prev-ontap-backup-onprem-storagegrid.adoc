...
[role="lead"]
Dans NetApp Backup and Recovery, vous pouvez sauvegarder les données de volume de vos systèmes ONTAP principaux sur site vers un système de stockage secondaire et vers le stockage objet StorageGRID. Utilisez cette page pour préparer votre environnement et activer les sauvegardes.
...
+
La nouvelle version de NetApp Backup and Recovery, qui permet de planifier et de créer des instantanés et des volumes répliqués, en plus de créer des sauvegardes sur le stockage objet, requiert que vous utilisiez la version 3.9.31 ou supérieure de l’agent Console. NetApp recommande d’utiliser cette dernière version pour gérer toutes vos sauvegardes.
...
Pour plus de détails sur DataLock et les exigences de protection contre les ransomwares pour StorageGRID, veuillez consulter link:prev-ontap-policy-object-options.html["Options de politique de sauvegarde sur objet"].

Versions de StorageGRID prises en charge:: StorageGRID 10.3 et versions ultérieures sont pris en charge.
+
--
Pour utiliser DataLock & la protection contre les ransomwares pour vos sauvegardes, vos systèmes StorageGRID doivent exécuter la version 11.6.0.3 ou une version supérieure.
...
Pour utiliser le stockage d'archives, un accès IP au nœud d'administration est nécessaire.
...
+
Si votre cluster utilise ONTAP 9.11.1 ou une version ultérieure, vous pouvez choisir de protéger vos sauvegardes contre la suppression et les attaques de ransomware en configurant _DataLock and Ransomware protection_. _DataLock_ protège vos fichiers de sauvegarde contre toute modification ou suppression, et _ransomware protection_ analyse vos fichiers de sauvegarde afin de détecter toute trace d’attaque de ransomware dans vos fichiers de sauvegarde.
