...
---
= Prérequis d'importation de SnapCenter pour NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Avant d’importer des ressources du SnapCenter Plug-in pour Microsoft SQL Server dans NetApp Backup and Recovery, assurez-vous que votre environnement répond à ces exigences supplémentaires.
...
== Vérifiez que l’hôte respecte les exigences du SnapCenter Plug-in

Pour importer des ressources à partir du SnapCenter Plug-in for Microsoft SQL Server, assurez-vous que les exigences de l’hôte pour l’installation du SnapCenter Plug-in for Microsoft SQL Server sont respectées.
...
** `1` : Les restrictions d'accès à distance de l'UAC sont désactivées. Les comptes locaux contournent le filtrage des jetons et disposent de privilèges d'administrateur complets lors d'un accès à distance.
...
----
. Vérifiez que la valeur actuelle est bien définie sur « 1 » dans  `LocalAccountTokenFilterPolicy` en exécutant :
+
[listing]
----
