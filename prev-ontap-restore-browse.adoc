...
[role="lead"]
Avec NetApp Backup and Recovery, restaurez les données ONTAP à l'aide de Parcourir et restaurer. Avant de restaurer, notez le nom du volume source, le système source, la machine virtuelle de stockage et la date du fichier de sauvegarde. Vous pouvez restaurer les données ONTAP à partir d'un instantané, d'un volume répliqué ou de sauvegardes stockées dans un stockage objet.
...
| NetApp StorageGRID | Système ONTAP sur site  a| 
* Système ONTAP sur site
* Cloud Volumes ONTAP
...
+
Notez que si vous sélectionnez un fichier de sauvegarde dans le stockage objet et que la protection contre les ransomwares est activée pour cette sauvegarde (si vous avez activé Datalock et la protection contre les ransomwares dans la règle de sauvegarde), vous serez invité à effectuer une analyse supplémentaire des ransomwares sur le fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d'analyser le fichier de sauvegarde à la recherche de ransomwares. (Vous engagerez des frais de sortie supplémentaires auprès de votre fournisseur de cloud pour accéder au contenu du fichier de sauvegarde.)
...
+
Si la protection contre les ransomwares est activée pour le fichier de sauvegarde (si vous avez activé Datalock et la protection contre les ransomwares dans la règle de sauvegarde), vous serez invité à effectuer une analyse supplémentaire du fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d'analyser le fichier de sauvegarde à la recherche de ransomwares. (Vous engagerez des frais de sortie supplémentaires auprès de votre fournisseur de cloud pour accéder au contenu du fichier de sauvegarde.)
