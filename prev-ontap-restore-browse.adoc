...
summary: 'Avec NetApp Backup and Recovery, restaurez les données ONTAP à l"aide de Browse and Restore.' 
---
= Restaurez les données ONTAP à l'aide de l'option Parcourir et restaurer dans NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez la fonction Parcourir et restaurer lorsque vous connaissez le nom du volume source, le système source, la machine virtuelle de stockage et la date du fichier de sauvegarde. Vous pouvez restaurer des données ONTAP à partir d'un instantané, d'un volume répliqué ou d'une sauvegarde stockée dans un stockage objet.
...
NOTE: La priorité de restauration élevée n'est pas prise en charge lors de la restauration de données depuis le stockage d'archivage Azure vers les systèmes StorageGRID.
...
Vous pouvez restaurer les données ONTAP à partir d'un fichier de sauvegarde situé dans un système secondaire (un volume répliqué) ou dans un stockage objet (un fichier de sauvegarde) vers les systèmes suivants.
...
| StorageGRID | Système ONTAP sur site  a| 
* Système ONTAP sur site
* Cloud Volumes ONTAP
...
| ONTAP S3 | Système ONTAP sur site  a| 
* Système ONTAP sur site
* Cloud Volumes ONTAP
...
| Magasins d'objets compatibles S3 | Système ONTAP sur site  a| 
* Système ONTAP sur site
* Cloud Volumes ONTAP
...
* Pour Amazon S3, l'agent Console peut être déployé dans AWS ou sur site
* Pour Azure Blob, l'agent de la Console peut être déployé dans Azure ou sur site
* Pour Google Cloud Storage, l'agent de la console doit être déployé dans votre VPC Google Cloud Platform
* Pour StorageGRID, l'agent Console doit être déployé sur site, avec ou sans accès Internet
* Pour ONTAP S3, l'agent de Console peut être déployé sur site (avec ou sans accès Internet) ou dans un environnement de fournisseur de cloud
...
NOTE: Lorsque vous restaurez un volume activé SnapLock, le volume restauré conserve la protection SnapLock uniquement si le stockage de destination est compatible WORM. Si vous restaurez vers une destination non compatible WORM, le statut SnapLock est supprimé du volume restauré.

Lors de la restauration d'une sauvegarde cloud vers un système Cloud Volumes ONTAP utilisant ONTAP 9.13.0 ou supérieur ou vers un système ONTAP sur site exécutant ONTAP 9.14.1, vous aurez la possibilité d'effectuer une opération de _restauration rapide_. La restauration rapide est idéale pour les situations de reprise après sinistre où vous devez fournir un accès à un volume le plus rapidement possible. Une restauration rapide restaure les métadonnées du fichier de sauvegarde vers un volume au lieu de restaurer l'intégralité du fichier de sauvegarde. La restauration rapide n'est pas recommandée pour les applications sensibles aux performances ou à la latence, et elle n'est pas prise en charge avec les sauvegardes dans un stockage d'archives.


NOTE: La restauration rapide est prise en charge pour les volumes FlexGroup uniquement si le système source à partir duquel la sauvegarde cloud a été créée exécutait ONTAP 9.12.1 ou une version ultérieure. Elle est prise en charge pour les volumes SnapLock uniquement si le système source exécutait ONTAP 9.11.0 ou une version ultérieure.
...
. Sélectionnez l'onglet *Restaurer* pour afficher le tableau de bord de restauration.
...
+
La colonne *Emplacement* indique si le fichier de sauvegarde (instantané) est *Local* (un instantané sur le système source), *Secondaire* (un volume répliqué sur un système ONTAP secondaire) ou *Stockage objet* (un fichier de sauvegarde dans le stockage objet). Choisissez le fichier que vous souhaitez restaurer.

. Sélectionnez *Suivant*.
+
Notez que si vous sélectionnez un fichier de sauvegarde dans le stockage objet et que la protection contre les ransomwares est activée pour cette sauvegarde (si vous avez activé DataLock et la protection contre les ransomwares dans la règle de sauvegarde), il vous sera demandé d'effectuer une analyse supplémentaire des ransomwares sur le fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d'analyser le fichier de sauvegarde à la recherche de ransomwares. (Vous encourrez des frais de sortie supplémentaires facturés par votre fournisseur de cloud pour accéder au contenu du fichier de sauvegarde.)
...
+
** Lors de la restauration depuis Amazon S3, sélectionnez l’IPspace dans le cluster ONTAP où le volume de destination résidera, saisissez la clé d’accès et la clé secrète de l’utilisateur que vous avez créé pour donner au cluster ONTAP l’accès au compartiment S3, et choisissez éventuellement un point de terminaison VPC privé pour un transfert de données sécurisé. Si vous utilisez un point de terminaison VPC privé, il prend le pas sur toute connexion proxy configurée.
...
. Vous pouvez activer l’option *Prioriser cette restauration par rapport à la file d’attente des tâches existantes* pour accorder une priorité plus élevée à cette opération de restauration. Lorsque cette option est activée, les autres tâches en cours peuvent être suspendues ou retardées afin de permettre à cette restauration de se dérouler.
. Sélectionnez *Restaurer*.
+
Vous retournez au tableau de bord de restauration afin de pouvoir consulter la progression de l'opération de restauration.
...
* Avec ONTAP 9.15.1, vous pouvez restaurer des dossiers FlexGroup à l'aide de l'option « Parcourir et restaurer ».
+
...
+
Si la protection contre les ransomwares est activée pour le fichier de sauvegarde (si vous avez activé DataLock et la protection contre les ransomwares dans la règles de sauvegarde), vous êtes invité à effectuer une analyse supplémentaire des ransomwares sur le fichier de sauvegarde avant de restaurer les données. Nous vous recommandons d’analyser le fichier de sauvegarde à la recherche de ransomwares. (Vous engagerez des frais de sortie supplémentaires auprès de votre fournisseur de cloud pour accéder au contenu du fichier de sauvegarde.)
...
+
Vous disposez de plusieurs options concernant l'emplacement lors de la restauration des dossiers et des fichiers.
...
*** Vous pouvez survoler un dossier et sélectionner l'extrémité de la ligne pour accéder aux sous-dossiers, puis sélectionner un dossier.


** Si vous avez sélectionné le même système et le même volume de destination que l’emplacement du dossier ou du fichier source, vous pouvez sélectionner *Conserver le chemin du dossier source* pour restaurer le dossier ou les fichiers dans le même dossier où ils existaient dans la structure source. Tous les dossiers et sous-dossiers identiques doivent déjà exister ; les dossiers ne sont pas créés. Lors de la restauration des fichiers à leur emplacement d’origine, vous pouvez choisir d’écraser les fichiers sources ou de créer de nouveaux fichiers.
