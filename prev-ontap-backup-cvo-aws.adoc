...
---
= Sauvegardez Cloud Volumes ONTAP sur Amazon S3 avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez NetApp Backup and Recovery pour sauvegarder les données de volume de vos systèmes Cloud Volumes ONTAP vers Amazon S3.
...
NOTE: Si vous utilisez un point de terminaison VPC privé pour les transferts de données de sauvegarde ou de restauration, celui-ci prime sur toute connexion proxy configurée.
...
Vous aurez également besoin d'un compte AWS pour l'espace de stockage où seront situées vos sauvegardes.
...
.Étapes
. Dans la page *Systems* de la NetApp Console, sélectionnez le cluster et sélectionnez *Enable* à côté de NetApp Backup and Recovery dans le panneau de droite.
+
Si la destination Amazon S3 de vos sauvegardes existe en tant que système sur la page *Systèmes*, vous pouvez faire glisser le cluster sur le système Amazon S3 pour lancer l’assistant de configuration.
...
+
Si la destination Amazon S3 de vos sauvegardes existe en tant que système sur la page *Systems* de la NetApp Console, vous pouvez faire glisser le cluster ONTAP sur le système Amazon S3.

** Dans la barre Sauvegarde et restauration, sélectionnez *Volumes*. Dans l'onglet Volumes, sélectionnez l'icône *Actions* image:icon-action.png["Icône Actions"] et choisissez *Activer la protection 3-2-1* pour un seul volume (dont la réplication ou la sauvegarde sur stockage objet n'est pas déjà activée).
...
+
Si vous choisissez d'utiliser vos propres clés gérées par le client, saisissez les informations de la clé.
...
. Vous pouvez éventuellement cocher la case *Corriger automatiquement les étiquettes incorrectes sur l’instantané local, la réplication et la sauvegarde*. Cela crée des instantanés avec une étiquette correspondant aux étiquettes des règles d’instantané, de réplication et de sauvegarde.
