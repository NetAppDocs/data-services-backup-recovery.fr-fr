...
---
= Sauvegardez Cloud Volumes ONTAP sur Google Cloud Storage avec NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utilisez NetApp Backup and Recovery pour sauvegarder les données de volume de vos systèmes Cloud Volumes ONTAP vers Google Cloud Storage.
...
Vous aurez également besoin d'un compte Google Cloud pour l'espace de stockage où seront situées vos sauvegardes.
...
Pour utiliser la fonctionnalité de recherche et de restauration NetApp Backup and Recovery, le rôle de l’agent de la Console doit inclure des autorisations spécifiques afin qu’il puisse accéder au service Google Cloud BigQuery. Consultez les autorisations ci-dessous et suivez les étapes si vous devez modifier la politique.
...
Vous pouvez activer NetApp Backup and Recovery sur un système Cloud Volumes ONTAP nouveau ou existant. La procédure diffère légèrement selon l’option choisie.
...
+
.. Sélectionnez *Modifier le projet* et choisissez un nouveau projet si celui que vous souhaitez utiliser est différent du projet par défaut (où se trouve l’agent de la Console).
...
. *Services* : Laissez NetApp Backup and Recovery activé et sélectionnez *Continuer*.
...
.Étapes
. Dans la page *Systèmes* de la NetApp Console, sélectionnez le système et sélectionnez *Activer* à côté de NetApp Backup and Recovery dans le panneau de droite.
+
...
Activez les sauvegardes à tout moment directement depuis votre système Cloud Volumes ONTAP.
...
+
Si la destination Google Cloud Storage de vos sauvegardes existe en tant que système sur la page *Systèmes* de la Console, vous pouvez faire glisser le cluster ONTAP sur le système Google Cloud Storage.
...
+
La page d'introduction de l'assistant présente les options de protection, notamment les instantanés locaux, la réplication et les sauvegardes. Si vous avez utilisé la deuxième option à cette étape, la page « Définir la stratégie de sauvegarde » s'affiche avec un volume sélectionné.
...
** Pour les règles de sauvegarde vers objet, configurez Datalock et la protection contre les ransomwares. Pour plus de détails sur Datalock et la protection contre les ransomwares, consultez link:prev-ontap-policy-object-options.html["Paramètres de la politique de sauvegarde sur objet"].
** Sélectionnez *Créer*.
...
+
Si vous choisissez d'utiliser vos propres clés gérées par le client, saisissez les informations relatives au porte-clés et à la clé.
...
. Cochez éventuellement la case pour *synchroniser automatiquement les étiquettes de la stratégie d'instantané avec les étiquettes des stratégies de réplication et de sauvegarde*. Cela crée des instantanés dont l'étiquette correspond à celles des stratégies de réplication et de sauvegarde.
