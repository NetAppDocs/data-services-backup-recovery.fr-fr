...
[role="lead"]
Avant de pouvoir protéger les banques de données VMware et les machines virtuelles exécutées sur les systèmes ONTAP, vous devez les découvrir dans NetApp Backup and Recovery. Si vous avez déjà installé le SnapCenter Plug-in for VMware vSphere, vous pouvez également importer ses données de sauvegarde et ses stratégies.
...
Si vous n'avez pas encore installé le plug-in SnapCenter pour VMware, ajoutez les informations vCenter afin que NetApp Backup and Recovery puisse découvrir les charges de travail. Dans chaque agent de la Console, sélectionnez les systèmes sur lesquels vous souhaitez découvrir les charges de travail.
...
Si vous avez déjà installé le SnapCenter Plug-in for VMware, importez les ressources du SnapCenter Plug-in dans NetApp Backup and Recovery en suivant ces étapes. La Console découvre les hôtes ESXi, les datastores, les machines virtuelles et les planifications à partir du Plug-in, vous n’avez donc pas à recréer ces informations.
...
NOTE: Si le SnapCenter Plug-in est déjà enregistré, vous pouvez mettre à jour les informations d'enregistrement existantes.
