...
[role="lead"]
Dans NetApp Backup and Recovery, créez vos propres politiques de sauvegarde KVM qui régissent la fréquence des sauvegardes, l'heure à laquelle la sauvegarde est effectuée et le nombre de fichiers de sauvegarde conservés.
...
+
*** *Activer les snapshots cohérents au niveau de la VM et cohérent au niveau des applications* : Activez cette option pour créer des snapshots cohérents au niveau de la VM et cohérent au niveau des applications. L'agent invité KVM QEMU doit être en cours d'exécution sur la VM. Si l'agent invité n'est pas en cours d'exécution, les snapshots seront cohérents après panne. Notez que l'activation de cette option peut augmenter la durée de sauvegarde et utiliser plus de stockage. De plus, la RAM active de la VM n'est pas incluse dans les snapshots cohérents.
